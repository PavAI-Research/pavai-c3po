...
from shared.audio.stt_vad import init_vad_model,init_vadaudio,has_speech_activity,normalize_audio_chunks
from .system_checks import PAVAI_APP_TALKIE,DEFAULT_TTS_VOICE_MODEL_TALKIE_AGENT,pavai_talkie_system_health_check
from shared.audio.voices_piper import espeak,convert_text_to_speech,librispeak
from shared.audio.vosk_client import api_speaker
from shared.aio.llmtokens import HistoryTokenBuffer
from shared.aio.llmchat import (system_prompt_assistant, llm_chat_completion,llm_chat)
...
def text_speaker_human(sd,text:str,output_voice:str="en_ryan",vosk_params=None):
    if _USE_VOICE_API:
...
def process_conversation(new_query:str,history:list, context_size:int=4096*2, 
    ...
    if len(history_buffer.overflow_summary)>0:
        ...
        assistant_response, history = llm_chat(user_prompt=new_query, history=history,
