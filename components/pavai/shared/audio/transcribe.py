...
#     print("\nTEST-Distilled-Whisper-2")
#     distilled_transcriber = DistrilledTranscriber(model_id_or_path="distil-whisper/distil-large-v2")
#     distilled_result = transcriber_client(abstract_transcriber=distilled_transcriber,
#                                 input_audio=input_audio_file,
#                                 input_type="audio",
#                                 task="transcribe")
#     print(distilled_result)

    # print("\nTEST-Faster-Whisper-3")
    # faster_transcriber = FasterTranscriber(model_id_or_path="large-v3")
    # faster_result = transcriber_client(abstract_transcriber=faster_transcriber,
    #                             input_audio=input_audio_file,
    #                             input_type="audio",
    #                             task="transcribe")
    # print(faster_result["transcription"],faster_result["language"])

    # print("")
    # transcriber_client(DistrilledTranscriber(model_id_or_path="openai"))
    # print("")
    # transcriber_client(FasterTranscriber(model_id_or_path="openai"))
    # print("")        
...
#     print("fn_transcribe started...")
#     faster_transcriber = FasterTranscriber(model_id_or_path="large-v3")
#     t0 = time.perf_counter()
#     text=""
#     words=0
#     if isinstance(new_chunk,str):
#         faster_result = transcriber_client(
#             abstract_transcriber=faster_transcriber,
#             input_audio=new_chunk,input_type="audio",task="transcribe")
#         #print(faster_result["transcription"],faster_result["language"])   
#         text= faster_result["transcription"]    
#     else:
#         sr, y = new_chunk
#         y = y.astype(np.float32)
#         y /= np.max(np.abs(y))          
...
#         faster_result = transcriber_client(
#             abstract_transcriber=faster_transcriber,
#             input_audio=stream,input_type="audio",task="transcribe")
#         text= faster_result["transcription"]

#     if text is not None and len(text)>0:
#         words=word_count(text) 
#     took_in_seconds = time.perf_counter()-t0    
#     status_msg=f"fn_transcribe completed. \nword count:[{words}] | took {took_in_seconds:.2f} seconds"    
#     print(status_msg)
#     gr.Info(status_msg)
#     return stream, text
