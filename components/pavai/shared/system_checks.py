    ...
    with Progress(transient=True) as progress: 
        try:       
            ...
            if _GLOBAL_SYSTEM_MODE=="SOLAR":
                ...
                local_llm_name_or_path = system_config["DEFAULT_LLM_MODEL_NAME_OR_PATH"]                
                local_llm_path = system_config["DEFAULT_LLM_MODEL_PATH"]
                local_llm_file = system_config["DEFAULT_LLM_MODEL_FILE"]
                local_llm_download = system_config["DEFAULT_LLM_MODEL_DOWNLOAD_PATH"]                
...
def system_sanity_tests(output_voice:str="en"):
    ...
    if "DEFAULT_SYSTEM_STARTUP_MSG_SYSTEM_CHECK_SUCCESS" not in system_config.keys():
        startup_message = DEFAULT_SYSTEM_STARTUP_MSG_SYSTEM_CHECK_SUCCESS    
    else:
        startup_message = system_config["DEFAULT_SYSTEM_STARTUP_MSG_SYSTEM_CHECK_SUCCESS"]    
    ...
    with Progress(transient=True) as progress:        
        ...
        try:        
           # 1.text-to-speech model
            ...
            test_audio_file=system_config["DEFAULT_SYSTEM_AUDIO_TEST_FILE"]         
            ...
            if "DEFAULT_SYSTEM_STARTUP_MSG_SYSTEM_CHECK_FAILED" not in system_config.keys():
                ...
                startup_message = system_config["DEFAULT_SYSTEM_STARTUP_MSG_SYSTEM_CHECK_FAILED"]
            if "DEFAULT_SYSTEM_STARTUP_MSG_SYSTEM_CHECK_FAILED_RECOVER" not in system_config.keys():
                failed_message = DEFAULT_SYSTEM_STARTUP_MSG_SYSTEM_CHECK_FAILED_RECOVER
            else:
                failed_message = system_config["DEFAULT_SYSTEM_STARTUP_MSG_SYSTEM_CHECK_FAILED_RECOVER"]
...
def pavai_talkie_system_health_check(output_voice:str="en_ryan"):
    System_ready = False
    try:
        environment_info()
